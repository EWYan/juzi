...
#define CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC 	0x2FAF080
...
#define TIMER_BASE_ADDR 		0xFF110000
#define TIMER_CLOCK_FREQUECY 	CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC
...
const int z_sys_timer_irq_for_test = DT_IRQN(DT_INST(0, xlnx_ttcps));
#endif
/*
...
static inline unsigned int sys_read32(unsigned int addr) {
    return *(volatile unsigned int *) addr;
}

static inline void sys_write32(unsigned int data, unsigned int addr) {
...
static unsigned int read_count(void)
{
	/* Read current counter value */
	return sys_read32(TIMER_BASE_ADDR + XTTCPS_COUNT_VALUE_OFFSET);
}

static void update_match(unsigned int cycles, unsigned int match)
{
	unsigned int delta = match - cycles;
...
// void sys_clock_set_timeout(int ticks, bool idle)
// {
// #ifdef CONFIG_TICKLESS_KERNEL
// 	unsigned int cycles;
...
// 	} else {
// 		next_cycles = cycles + ((unsigned int)ticks * CYCLES_PER_TICK);
// 	}
...
unsigned int sys_clock_cycle_get_32(void)
{
...
int sys_clock_driver_init(void)
{
	...
	extern void irq_connect(unsigned int irq, unsigned int priority, void *arg, void (*isr)(const void *));
	/* Connect timer interrupt */
	irq_connect(TIMER_IRQ, 0, 0, ttc_isr);
