...
#include "gic.h"
__attribute((used)) const unsigned int ulICCIAR = 0xF900100C;
__attribute((used)) const unsigned int ulICCEOIR = 0xF9001010;
...
static inline unsigned char sys_read8(unsigned int addr) {
    return *(volatile unsigned char *) addr;
}

static inline void sys_write8(unsigned char data, unsigned int addr) {
    *(volatile unsigned char *) addr = data;
}

static const unsigned long long cpu_mpid_list[] = {
	0
};
...
unsigned char arm_gic_irq_is_enabled(unsigned int irq)
{
	int int_grp, int_off;
...
unsigned char arm_gic_irq_is_pending(unsigned int irq)
{
	int int_grp, int_off;
	unsigned int enabler;
...
#if 1
void gic_raise_sgi(unsigned int sgi_id, unsigned long long target_aff,
		unsigned short target_list)
{
	unsigned int sgi_val;
...
// void test_irq(void) {
// 	gic_raise_sgi(0x44, 0, 0);
// }
static void gic_dist_init(void)
{
	unsigned int gic_irqs, i;
	unsigned char cpu_mask = 0;
