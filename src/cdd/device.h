...
struct device_state {
	 ...
	 * positive result value in a unsigned char type.
	 */
	unsigned char init_res;
 ...
 * @see device_is_ready()
 */
unsigned char z_device_is_ready(const struct device *dev);
 ...
 * is passed as argument.
 */
__syscall unsigned char device_is_ready(const struct device *dev);

static inline unsigned char z_impl_device_is_ready(const struct device *dev)
{
